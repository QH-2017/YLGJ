...
public class AiController {
    ...
    public Result chat(@RequestBody Map<String, Object> request) {
        ...
        if (userMessage == null || userMessage.trim().isEmpty()) {
            return new Result(false, "请输入您的问题");
        }

        if (apiKey == null || apiKey.trim().isEmpty()) {
            // 未配置API Key时使用本地智能回复
            return new Result(true, "回复成功", getLocalReply(userMessage));
        }
        ...
        try {
            ...
            if (choices != null && choices.size() > 0) {
                String content = choices.getJSONObject(0)
                    ...
                    .getString("content");
                return new Result(true, "回复成功", content.trim());
            }

            return new Result(false, "AI回复解析失败");
        } catch (Exception e) {
            return new Result(false, "AI服务暂时不可用：" + e.getMessage());
        }
    }
